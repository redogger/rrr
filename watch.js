 ...
 *  ---------------------------------------------------------------------------
 *  Key design:
 *   • ONE persistent page in authenticated context (kept open)
 *   • Fetch runs INSIDE the page (page.evaluate) → exact same request as DULMS
 *   • Early detection every 10 seconds
 *   • Sniper every 10 seconds (once target resolved)
...
const USER_CONFIG = {
  ...
  earlyDetectionIntervalMs: 10 * 1000,     // ⭐ 10 ثواني
...
const BOT_COMMANDS = [
  ...
  { command: 'early',    description: '🕐 Early detection on/off/status' },
...
// ═══════════════════════════════════════════════════════════════════════════
//  API CLIENT — inside page context (fast + authenticated)
// ═══════════════════════════════════════════════════════════════════════════
function makeApi(ctx, pageRef) {
  let courseCache = { ts: 0, data: null };
  const CACHE_TTL = 8 * 1000;   // 8s cache (less than 10s interval)

  // ⭐ Fetch via the persistent authenticated page — same headers/cookies as DULMS
...
async function earlyDetectionCheck(api, state) {
  ...
  if (!found) {
    if (state.earlyDetection.checks % 6 === 0) {  // كل دقيقة log
  ...
  await tgSend(msg, { silent: false });   // 🔔 with sound
...
async function runScan(browser, deadline) {
  ...
  // ⭐ Persistent page — opened once, kept alive
  ...
  while (Date.now() < deadline) {
    ...
    // Page health check — reload if crashed
    ...
    // Sniper every 10s (once target resolved)
